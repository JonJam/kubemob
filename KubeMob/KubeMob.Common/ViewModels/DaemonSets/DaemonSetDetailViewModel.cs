...
namespace KubeMob.Common.ViewModels.DaemonSets
{
    ...
    public class DaemonSetDetailViewModel : ViewModelBase
    {
        private readonly IKubernetesService kubernetesService;
        private readonly IPopupService popupService;

        private string name;
        private DaemonSetDetail detail;
        ...
        public override async Task Initialize(object navigationData)
        {
            ...
            await this.PerformNetworkOperation(async () =>
            {
                try
                {
                    this.Detail = await this.kubernetesService.GetDaemonSetDetail(objectId.Name, objectId.NamespaceName);
                }
