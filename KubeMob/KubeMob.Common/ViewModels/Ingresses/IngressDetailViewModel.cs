...
namespace KubeMob.Common.ViewModels.Ingresses
{
    ...
    public class IngressDetailViewModel : ViewModelBase
    {
        private readonly IKubernetesService kubernetesService;
        private readonly IPopupService popupService;

        private string name;
        private IngressDetail detail;
        ...
        public override async Task Initialize(object navigationData)
        {
            ...
            await this.PerformNetworkOperation(async () =>
            {
                try
                {
                    this.Detail = await this.kubernetesService.GetIngressDetail(objectId.Name, objectId.NamespaceName);
                }
