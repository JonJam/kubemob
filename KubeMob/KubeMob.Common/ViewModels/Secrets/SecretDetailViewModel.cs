...
namespace KubeMob.Common.ViewModels.Secrets
{
    ...
    public class SecretDetailViewModel : ViewModelBase
    {
        private readonly IKubernetesService kubernetesService;
        private readonly IPopupService popupService;

        private string name;
        private SecretDetail detail;
        ...
        public override async Task Initialize(object navigationData)
        {
            ...
            await this.PerformNetworkOperation(async () =>
            {
                try
                {
                    this.Detail = await this.kubernetesService.GetSecretDetail(objectId.Name, objectId.NamespaceName);
                }
